...
for(i in 1:50) {
  ...
  OD <- readNIfTI(paste(folder_withdata,subjects[i],"/ses-t0/dwi/",subjects[i],"_ses-t0_space-individual_desc-10NODDIfitISOVF_dwi.nii.gz",sep = ""), reorient = FALSE)
...
saveRDS(subs_df,file = "C:/Users/vissa/Documents/Universitaeten/TEC/Clínicas/2021/UNAM/TMS/postprocess_HARDI/NODDI_all/ROIs_new/ISOVF/meanISOVFinROIs_new.RDS")
...
plots_violin <- ggplot(subs_df, aes(fill=group,x=atlas,y=val)) + 
  ...
  #geom_jitter() +
  ggtitle("CSF volume fraction (ISOVF): sham and active groups at 0 weeks") +
...
plots_violin
ggsave(plots_violin, file='C:/Users/vissa/Documents/Universitaeten/TEC/Clínicas/2021/UNAM/TMS/postprocess_HARDI/NODDI_all/ROIs_new/ISOVF/ISOVF_WMregions_shamVSactive.png', width = 1366/72, height = 768/72, dpi = 72)
...
gt_tbl <- gt(data = t_tests_tib, auto_align = FALSE) %>%
  tab_header("CSF volume fraction (ISOVF): sham and active groups at 0 weeks")
...
gt_tbl %>%
  gtsave("ISOVF_t_tests.html", inline_css = TRUE,
         path = 'C:/Users/vissa/Documents/Universitaeten/TEC/Clínicas/2021/UNAM/TMS/postprocess_HARDI/NODDI_all/ROIs_new/ISOVF/'
  )
